#!/usr/bin/env python3
"""
Module để áp dụng trọng số tích hợp đã cải tiến vào estimator
"""
...
def apply_weight_factors_to_estimator():
    """
    Tạo một file tích hợp để áp dụng các hệ số trọng số mới
    vào estimator
    """
    # Đường dẫn đến file tích hợp
    integration_path = os.path.join(PROJECT_ROOT, "requirement_analyzer", "model_integration.py")
    
    # Nội dung file tích hợp
    content = '''
import os
import json
from pathlib import Path
...
'''
    ...
    print(f"Đã tạo file tích hợp trọng số tại {integration_path}")

def create_model_weight_integration():
    """
    Tạo script để tích hợp trọng số mô hình vào estimator.py
    """
    # Đường dẫn đến file tích hợp
    integration_script_path = os.path.join(PROJECT_ROOT, "requirement_analyzer", "task_integration.py")
...
'''
    ...
    print(f"Đã tạo script tích hợp trọng số mô hình tại {integration_script_path}")

def main():
    """Hàm chính để áp dụng trọng số tích hợp"""
    print("Bắt đầu áp dụng trọng số tích hợp...")
    
    # Tạo file tích hợp trọng số
    apply_weight_factors_to_estimator()
    
    # Tạo script tích hợp với estimator
    create_model_weight_integration()
    
    print("Hoàn thành áp dụng trọng số tích hợp!")
    print("Chạy script task_integration.py để tích hợp vào estimator.py")
