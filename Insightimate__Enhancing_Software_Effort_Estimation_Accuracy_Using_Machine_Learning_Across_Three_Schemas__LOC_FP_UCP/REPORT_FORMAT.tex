...
\hline
Table 1 & \pageref{tab:dataset-manifest} & Dataset Provenance: Source Identifiers, Sample Sizes, and Deduplication Statistics \\
\hline
Table 2 & \pageref{tab:overall-results} & Overall Performance Comparison: Macro-Averaged Metrics Across All Models and Schemas \\
\hline
Table 3 & \pageref{tab:per-schema} & Per-Schema Results: Random Forest Performance Breakdown by LOC, FP, and UCP \\
\hline
Table 4 & \pageref{tab:feature-importance} & Feature Importance Breakdown: Relative Contribution of Project Size and Secondary Factors \\
\hline
Table 5 & \pageref{tab:ablation} & Ablation Study Components: MMRE Impact of Individual Preprocessing Steps \\
\hline
Table 6 & \pageref{tab:loso} & Cross-Source Generalization: Leave-One-Source-Out Validation Results by LOC Source \\
\hline
Table 7 & \pageref{tab:stat-tests} & Statistical Significance Tests: Wilcoxon Signed-Rank p-values with Holm-Bonferroni Correction \\
\hline
Table 8 & \pageref{tab:hyperparams} & Hyperparameter Configuration: Model-Specific Settings for LOC, FP, and UCP Schemas \\
\hline
Table 9 & \pageref{tab:bootstrap-ci} & Confidence Intervals: 95\% Bootstrap CI for Function Points Predictions \\
\hline
Table 10 & \pageref{tab:unit-conversion} & Unit Conversion Reference: Effort and Size Normalization Factors across Schemas \\
\hline
\end{tabular}
...
Fit via non-linear least squares on training data using \texttt{scipy.optimize.curve\_fit}
...
\begin{table}[h]
\centering
\caption{Dataset Provenance: Source Identifiers, Sample Sizes, and Deduplication Statistics}
\label{tab:dataset-manifest}
\footnotesize
\begin{tabular}{|l|c|c|c|c|c|}
\hline
\textbf{Source} & \textbf{Schema} & \textbf{Period} & \textbf{Raw} & \textbf{Final} & \textbf{Dedup \%} \\
\hline
\multicolumn{6}{|c|}{\textit{LOC Schema (11 sources)}} \\
\hline
...
\hline
\multicolumn{6}{|c|}{\textit{FP Schema (4 sources)}} \\
\hline
...
\hline
\multicolumn{6}{|c|}{\textit{UCP Schema (3 sources)}} \\
\hline
Silhavy et al.\ & UCP & 2009--2019 & 72 & 68 & $-5.6\%$ \\
Anda et al.\ & UCP & 2001--2010 & 41 & 38 & $-7.3\%$ \\
Academic UCP & UCP & 1993--2023 & 26 & 25 & $-3.8\%$ \\
\hline
\textbf{Total} & \textbf{3} & \textbf{1979--2023} & \textbf{3,290} & \textbf{3,054} & \textbf{$-7.2\%$} \\
\hline
\end{tabular}
\end{table}

Full rebuild scripts and raw-to-clean processing code available at: \url{https://github.com/Huy-VNNIC/AI-Project}
...
\centering
\begin{tikzpicture}[
    pstep/.style={rectangle, draw, minimum width=3.5cm, minimum height=0.8cm, font=\small\bfseries, align=center},
    data/.style={rectangle, draw, minimum width=2.5cm, minimum height=0.7cm, font=\tiny, align=center}
]

% Raw data
\node at (0,9) [pstep, fill=blue!30] (raw) {Raw Project Data\\(3,054 projects)};

% Step 1: Unit Harmonization
\node at (0,7.5) [pstep, fill=yellow!20] (harm) {Step 1: Unit\\Harmonization};
\node at (3.5,7.5) [data, fill=blue!10] {Effort $\to$ PM\\LOC $\to$ KLOC};

% Step 2: Missing Value Handling
\node at (0,6) [pstep, fill=yellow!20] (missing) {Step 2: Missing\\Value Handling};
\node at (3.5,6) [data, fill=blue!10] {Median Imputation\\(features only)};

% Step 3: Outlier Detection
\node at (0,4.5) [pstep, fill=yellow!20] (outlier) {Step 3: Outlier\\Detection \& Capping};
\node at (3.5,4.5) [data, fill=blue!10] {IQR-based\\(threshold: 1.5×IQR)};

% Step 4: Log Transformation
\node at (0,3) [pstep, fill=yellow!20] (log) {Step 4: Log\\Transformation};
\node at (3.5,3) [data, fill=blue!10] {log1p(x)\\(reduce skewness)};

% Step 5: Normalization
\node at (0,1.5) [pstep, fill=yellow!20] (norm) {Step 5:\\Normalization};
\node at (3.5,1.5) [data, fill=blue!10] {Scale to [0,1]\\or Standardize};

% Clean data
\node at (0,0) [pstep, fill=green!30] (clean) {Clean Dataset\\(Ready for Training)};
...
\begin{table}[h]
\centering
\caption{Overall Performance Comparison: Macro-Averaged Metrics Across All Models and Schemas}
\label{tab:overall-results}
\footnotesize
\begin{tabular}{|l|c|c|c|c|c|c|c|}
\hline
\textbf{Model} & \textbf{MMRE} & \textbf{MdMRE} & \textbf{PRED(25)} & \textbf{MAE (PM)} & \textbf{MdAE} & \textbf{RMSE} & \textbf{$R^2$} \\
\hline
Calibrated Baseline & $1.120$ & $0.891$ & $0.182$ & $18.45 \pm 1.2$ & $14.2$ & $32.8$ & $0.42$ \\
...
\textbf{Random Forest} & $\mathbf{0.647}$ & $\mathbf{0.498}$ & $\mathbf{0.395}$ & $\mathbf{12.66}$ & $\mathbf{9.6}$ & $\mathbf{22.8}$ & $\mathbf{0.71}$ \\
\hline
\end{tabular}
\end{table}

\textbf{Key Finding:} Random Forest achieves 42\% MMRE improvement over calibrated baseline ($1.12 \to 0.647$) and reduces MAE from 18.45 PM to 12.66 PM. PRED(25) improves from 0.182 to 0.395, meaning nearly 40\% of predictions fall within 25\% of actual effort.
...
\caption{Model Performance Comparison: Random Forest Achieves Best Macro-Averaged Results (MAE in Person-Months). RF achieves 42\% improvement over the calibrated size-only power-law baseline.}
...
\begin{table}[h]
\centering
\caption{Per-Schema Results: Random Forest Performance Breakdown by LOC, FP, and UCP (mean $\pm$ std over 10 random seeds)}
\label{tab:per-schema}
\begin{tabular}{|l|c|c|c|c|c|c|}
\hline
\textbf{Schema} & \textbf{n} & \textbf{MMRE} & \textbf{MdMRE} & \textbf{PRED(25)} & \textbf{MAE (PM)} & \textbf{$R^2$} \\
\hline
LOC & 2,765 & $0.645 \pm 0.038$ & $0.491$ & $0.401$ & $11.80 \pm 0.72$ & $0.72$ \\
FP & 158 & $0.651 \pm 0.051$ & $0.508$ & $0.381$ & $12.20 \pm 1.42$ & $0.68$ \\
UCP & 131 & $0.652 \pm 0.047$ & $0.511$ & $0.396$ & $14.02 \pm 1.18$ & $0.69$ \\
\hline
\textbf{Macro-Avg.} & --- & $\mathbf{0.647 \pm 0.041}$ & $\mathbf{0.498}$ & $\mathbf{0.395}$ & $\mathbf{12.66 \pm 0.85}$ & $\mathbf{0.71}$ \\
\hline
\end{tabular}
\end{table}
...
Random Forest: MMRE $0.645 \pm 0.038$, MAE 11.8 PM, $R^2$ 0.72. Consistent performance across all 11 LOC sources. Less sensitive to outliers than parametric baseline due to ensemble averaging.

\subsubsection{FP Schema (n=158)}

Random Forest: MMRE 0.651, MAE 12.2 PM [95\% CI: 10.2--15.8]. Bootstrap CIs confirm non-artifactual superiority despite small sample. Macro-averaging prevents FP from being dominated by LOC. Results should be treated as \textbf{exploratory} given the limited sample size.
...
\caption{Per-Schema Results: Random Forest Performance Breakdown by LOC, FP, and UCP Sizing Schemas. Macro-averaging ensures equal contribution from each schema regardless of sample size.}
...
\foreach \x/\lbl in {0/{0\%}, 1/{5\%}, 2/{10\%}, 3/{15\%}, 4/{20\%}, 5/{25\%}, 6/{30\%}} {
  \draw [thin, gray] (\x,3.5) -- (\x,9);
  \node [below, font=\tiny] at (\x,3.3) {\lbl};
}
...
\begin{table}[h]
\centering
\caption{Cross-Source Generalization: Leave-One-Source-Out Validation Results by LOC Source}
\label{tab:loso}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Hold-Out Source} & \textbf{Projects (n)} & \textbf{MAE Degradation} & \textbf{Assessment} \\
\hline
NASA93 & 93 & 18\% & Good \\
COCOMO81 & 63 & 21\% & Good \\
ISBSG & 364 & 19\% & Good \\
...
\hline
\textbf{Average} & \textbf{---} & \textbf{21\%} & \textbf{Acceptable} \\
\hline
\end{tabular}
\end{table}

\textbf{Finding:} 21\% average degradation is acceptable for production systems. Models trained on 10 out of 11 LOC sources generalize reasonably to unseen sources, confirming cross-source transferability. No single source produces catastrophic degradation ($>25\%$), validating the framework's generalization capability.
...
\begin{table}[h]
\centering
\caption{Ablation Study Components: MMRE Impact of Individual Preprocessing Steps}
\label{tab:ablation}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Configuration} & \textbf{MMRE} & \textbf{MAE (PM)} & \textbf{Improvement} \\
\hline
...
\textbf{Finding:} Calibration alone yields 14.5\% MMRE improvement. Log-transform is the largest individual contributor ($-$29.3\%). The full pipeline combining all components with RF achieves a cumulative $-$43\% improvement over the uncalibrated COCOMO baseline.
...
\begin{table}[h]
\centering
\caption{Feature Importance Breakdown: Relative Contribution of Project Size and Secondary Factors (Random Forest, averaged across 10 seeds)}
\label{tab:feature-importance}
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Feature} & \textbf{LOC Schema} & \textbf{FP Schema} & \textbf{UCP Schema} & \textbf{Average} \\
\hline
...
\caption{Random Forest Feature Importance: Project Size Dominates Prediction ($\approx$68\%) Across All Schemas, with Secondary Contributions from Duration, Team Size, and Experience.}
...
\begin{table}[h]
\centering
\caption{Statistical Significance Tests: Wilcoxon Signed-Rank p-values with Holm-Bonferroni Correction ($\alpha = 0.05$)}
\label{tab:stat-tests}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Model Comparison} & \textbf{p-value} & \textbf{Effect Size (Cliff's $\delta$)} & \textbf{Significant?} \\
\hline
...
\subsection{3.8 Hyperparameter Configuration and Additional Tables}

\begin{table}[h]
\centering
\caption{Hyperparameter Configuration: Model-Specific Settings for LOC, FP, and UCP Schemas}
\label{tab:hyperparams}
\begin{tabular}{|l|l|l|}
\hline
\textbf{Model} & \textbf{Hyperparameter} & \textbf{Value (all schemas)} \\
\hline
\multirow{2}{*}{Linear Regression} & Fit intercept & True \\
 & Log-log transform & Enabled \\
\hline
\multirow{2}{*}{Decision Tree} & max\_depth & 10 \\
 & min\_samples\_split & 5 \\
\hline
\multirow{3}{*}{Random Forest} & n\_estimators & 100 \\
 & max\_depth & 15 \\
 & min\_samples\_leaf & 2 \\
\hline
\multirow{3}{*}{Gradient Boosting} & n\_estimators & 100 \\
 & learning\_rate & 0.1 \\
 & max\_depth & 5 \\
\hline
...
\hline
\multirow{2}{*}{Calibrated Baseline} & Model type & Power-law $E = A \cdot \text{Size}^B$ \\
...
\begin{table}[h]
\centering
\caption{Confidence Intervals: 95\% Bootstrap CI for Function Points Predictions (Random Forest, 1000 resamples)}
...
\begin{table}[h]
\centering
\caption{Unit Conversion Reference: Effort and Size Normalization Factors Across Schemas}
\label{tab:unit-conversion}
\begin{tabular}{|l|l|l|l|}
\hline
\textbf{Schema} & \textbf{Size Unit} & \textbf{Effort Unit} & \textbf{Conversion Applied} \\
\hline
...
\hline
\textbf{Raw unit} & \textbf{Converted to} & \textbf{Factor} & \textbf{Note} \\
\hline
...
Beyond the stated assumptions, several validity threats may affect the interpretation and generalizability of our findings. We categorize them following standard empirical software engineering practice into \textit{internal}, \textit{external}, \textit{construct}, and \textit{conclusion} validity.
...
Effort and size metrics inherently vary across organizations---from person-hours to adjusted person-months---and may embed subjectivity in Function Point or Use Case Point estimation. Although the harmonization framework standardizes units (1 PM = 160 hours), measurement bias remains possible. To address metric limitations (e.g., MMRE bias toward underestimates), we complement MRE-based metrics with absolute-error (MAE, RMSE) and variance-explained ($R^2$) measures.

\subsection{Conclusion Validity}
Statistical inference reliability was reinforced through Wilcoxon signed-rank tests with Holm--Bonferroni correction and effect-size reporting via Cliff's $\delta$. Nevertheless, multiple comparisons can increase Type II error risk, especially for limited-sample schemas (FP, $n{=}158$). Significance should be interpreted as indicative rather than definitive for small schemas.

\subsection{Specific Limitations}

\textbf{Function Point Schema Limitations:} The FP dataset ($n{=}158$) limits statistical power. Results are mitigated through LOOCV and bootstrap CIs, but FP findings should be considered \textbf{exploratory}.

\textbf{Calibrated Baseline Constraints:} Our power-law baseline uses only size metrics without full COCOMO~II cost drivers, representing a \textit{lower bound} for parametric methods, not COCOMO~II's full potential in industrial settings with available driver data.

\textbf{Cross-Schema Transfer Not Attempted:} Models are trained independently per schema (LOC/FP/UCP) to avoid semantic feature mismatch between fundamentally different sizing paradigms.

\textbf{Modern DevOps Underrepresentation:} Public datasets are biased toward pre-2010 waterfall/iterative projects. Our preprocessing and validation methodology is dataset-agnostic and directly applicable to future DevOps-era corpora.
...
\textbf{Parametric Models:} COCOMO~\cite{boehm2000cocomo}, SEER-SEM, and SLIM dominated early research, offering interpretability via explicit cost drivers but suffering from rigid power-law forms and limited adaptability to heterogeneous projects. These models require extensive calibration data and assume separable multiplicative effects that may not hold across modern project types.

\textbf{Ensemble Methods:} Random Forest~\cite{breiman2001random}, Gradient Boosting~\cite{friedman2001greedy}, and XGBoost~\cite{chen2016xgboost} became dominant in empirical SEE research for tabular regression. They handle non-linearity and feature interactions robustly without requiring explicit cost driver specification. However, reproducibility in terms of provenance documentation, deduplication, and aggregation often remains unclear.

\textbf{Deep Learning Approaches:} Neural networks with embedding layers and LSTM architectures explore representation learning for rich telemetry datasets. While promising for issue-tracking and sprint-based data, they face reproducibility challenges and limited applicability to small public benchmarks ($n < 3000$).

\textbf{Transfer Learning and Cross-Project Models:} Analogy-based and cross-company estimation methods address cold-start problems by leveraging external data. Feature alignment and empirical gains remain mixed, particularly for cross-schema transfer where sizing paradigms are fundamentally incompatible.

\textbf{Uncertainty-Aware Methods:} Recent hybrid approaches combining parametric structure with non-parametric flexibility offer confidence intervals and fuzzy parameter handling, but require substantial feature engineering and proprietary organizational metadata.

\subsection{Comparison with Prior Work}

\begin{table}[h]
\centering
\caption{Comparison with Representative SEE Studies Across Schemas, Evaluation Protocols, and Reproducibility}
\label{tab:related-compare}
\footnotesize
\begin{tabular}{|p{2.8cm}|c|p{2.5cm}|c|c|}
\hline
\textbf{Study} & \textbf{Schema} & \textbf{Models} & \textbf{Reproducible?} & \textbf{Best MMRE} \\
\hline
Wen et al.\ (2012) & LOC & SVM, ANN, RF & Partial & 0.52--0.95 \\
Kocaguneli et al.\ (2012) & LOC & Analogy-based (k-NN) & Partial & $\approx$0.65 \\
Minku \& Yao (2013) & LOC & Bagging, Boosting & Partial & $\approx$0.62 \\
Azzeh \& Nassif (2019) & LOC, FP & RF + feature selection & Partial & $\approx$0.51 \\
Alqadi et al.\ (2021) & LOC & Deep NN, hybrid & No & $\approx$0.58 \\
Pandey et al.\ (2023) & LOC, FP & RF, XGBoost, LightGBM & Partial & $\approx$0.48 \\
\hline
\textbf{This work} & LOC+FP+UCP & LR, DT, RF, GB, XGB + calibrated baseline & \textbf{Yes (code+data+rebuild)} & \textbf{0.647} \\
\hline
\end{tabular}
\end{table}
...
While prior studies explore stronger learners (ensembles, deep models), our work targets three methodological gaps: (i) \textbf{dataset provenance}---complete manifest with source URLs, DOIs, deduplication rules, and rebuild scripts; (ii) \textbf{fair parametric baseline}---size-only power-law calibrated on training data per schema and seed, avoiding straw-man COCOMO~II comparisons; and (iii) \textbf{explicit aggregation}---macro-averaged (equal weight per schema) and micro-averaged metrics preventing LOC dominance, with LOOCV and bootstrap CI for small-sample FP. These three contributions provide a transparent, auditable template for future SEE benchmarking.
