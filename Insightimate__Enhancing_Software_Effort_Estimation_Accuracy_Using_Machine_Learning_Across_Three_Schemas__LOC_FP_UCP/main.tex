...
We evaluate five representative methods (LR, DT, RF, GB, XGBoost) selected to span the complexity spectrum from simple linear baselines to modern ensemble learners. While other gradient boosting variants exist (LightGBM~\cite{ke2017lightgbm}, CatBoost~\cite{prokhorenkova2018catboost}), they share similar algorithmic foundations (gradient-based tree boosting) and typically achieve comparable performance on tabular data~\cite{prokhorenkova2018catboost}. Our focus is establishing a \textbf{benchmarking methodology} (manifest, baseline, protocols) rather than exhaustive model comparison; future work can apply this framework to evaluate LightGBM/CatBoost or domain-specific architectures.

\paragraph{Cross-Schema Transfer Not Attempted.}
Models are trained \textbf{independently per schema} (LOC/FP/UCP) without cross-schema transfer learning, multi-task learning, or shared representation learning. This design choice is intentional, not a limitation: LOC, FP, and UCP represent fundamentally different sizing philosophies with distinct feature semantics (e.g., KLOC measures code volume, FP measures functional complexity, UCP measures use-case actors/scenarios). Pooling heterogeneous schemas for joint training risks \textbf{semantic feature mismatch}—conflating unrelated predictor spaces and degrading performance on all schemas.

\textbf{Rationale for schema-specific training:}
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Feature space incompatibility}: LOC projects have different auxiliary features (language, domain) than FP projects (transaction types, complexity adjustments) and UCP projects (actor classifications, technical factors). Direct pooling without careful feature alignment creates false equivalences.
    \item \textbf{Prevents information leakage}: Schema-partitioned training ensures models cannot exploit cross-schema correlations that would not exist in deployment (where schema choice is typically project-specific and predetermined).
    \item \textbf{Interpretability}: Schema-specific models allow direct comparison of sizing paradigms under controlled conditions, supporting the paper's benchmarking objective.
\end{itemize}

\textbf{Future work: Cross-schema transfer learning.}
While outside the scope of this benchmarking study, cross-schema transfer represents a promising research direction requiring:
\begin{enumerate}[leftmargin=1.5em]
    \item \textbf{Feature alignment strategies}: Meta-learning representations, domain-invariant embeddings, or explicit semantic mapping functions to translate between LOC/FP/UCP feature spaces.
    \item \textbf{Multi-task learning}: Joint optimization with schema-specific output heads, sharing lower-layer representations while maintaining schema-specific decision boundaries.
    \item \textbf{Evaluation protocols}: Leave-one-schema-out validation to assess zero-shot transfer, and few-shot adaptation experiments to quantify sample efficiency gains.
\end{enumerate}

Our schema-specific approach establishes \textit{baseline performance} for future transfer learning studies to demonstrate improvement over independent schema-specific training.

\paragraph{Deduplication and Leakage Risks.}
Despite rigorous deduplication rules (Table~\ref{tab:dataset-summary}, detailed in Table S1), residual near-duplicates may persist due to:
\begin{itemize}[leftmargin=1.5em]
    \item Projects re-reported across multiple publications with slight measurement variations;
    \item Forked or iterative project versions that share substantial code/effort characteristics;
    \item Incomplete metadata preventing perfect matching (e.g., missing project names in older sources).
\end{itemize}
We control for exact duplicates (name + size + effort) and report deduplication counts transparently, but \textit{semantic overlap} (e.g., similar-domain projects) cannot be fully eliminated from public corpora without access to proprietary repository histories.

\paragraph{Modern DevOps Underrepresentation.}
Public datasets span 1993--2022 but are biased toward legacy waterfall/iterative projects. Agile/DevOps environments with continuous integration, microservices, and sprint-based effort tracking may exhibit different scaling behaviors not captured by traditional LOC/FP/UCP metrics. \textbf{Data availability constraints:} We attempted to incorporate modern DevOps telemetry (GitHub Actions logs, Jira sprint data, CI/CD metrics) but encountered systematic barriers: (i) most organizational effort data remains proprietary due to competitive sensitivity, (ii) public repositories (e.g., GitHub) provide commit/issue metadata but lack \emph{ground-truth effort} labels required for supervised learning, and (iii) existing DevOps studies~\cite{fox2017devops,chen2020devops} typically report aggregate statistics rather than project-level datasets suitable for benchmarking. Generalization to modern contexts requires telemetry-enriched datasets (commit velocity, deployment frequency, lead time, mean time to recovery) with validated effort annotations—an open challenge for the community.

\textbf{Framework applicability:} Despite data limitations, our preprocessing pipeline (unit harmonization, outlier control, schema-specific protocols), calibrated baseline methodology, and cross-source validation (LOSO) are \emph{dataset-agnostic} and directly applicable to future industrial and DevOps corpora. The contribution is methodological infrastructure for fair benchmarking, not definitive SOTA claims on modern contexts.
...
\subsection{Parametric Models (COCOMO, SEER, SLIM)}

Parametric models—COCOMO~\cite{boehm1981software}, SEER-SEM, SLIM—dominated early SEE research.
\textbf{Strengths:} Interpretability via explicit cost drivers (team experience, complexity, tool support); managerial communication; decades of industrial validation.
\textbf{Limitations:} Most public FP/UCP datasets lack full cost-driver annotations; applying default parameters creates straw-man baselines; fixed power-law forms underfit heterogeneous modern projects.
\textbf{How our work differs:} We use a \emph{calibrated} size-only power-law baseline (Eq.~\ref{eq:baseline-calibrated}) fitted on training data per schema and seed, ensuring fair comparison when drivers are missing.

\subsection{Ensemble Methods and Gradient Boosting}

Since the 2000s, ensemble tree methods—Random Forest~\cite{breiman2001random}, Gradient Boosting~\cite{friedman2001greedy}, XGBoost~\cite{chen2016xgboost}, LightGBM~\cite{ke2017lightgbm}, CatBoost~\cite{prokhorenkova2018catboost}—have become dominant for tabular regression.
\textbf{Strengths:} Handle non-linearity and feature interactions; robust to irrelevant features; strong empirical performance on SEE benchmarks~\cite{pandey2023comprehensive}.
\textbf{Limitations:} Vulnerable to target leakage if features derived from effort/time; hyperparameter tuning and model selection can bias comparisons; reproducibility often unclear (provenance, deduplication, aggregation).
\textbf{How our work differs:} We provide full dataset manifest (Table~\ref{tab:dataset-summary}, details in Table S1) with explicit deduplication and leakage controls; macro-averaging across schemas prevents LOC dominance; LOSO validation tests cross-source generalization; ablation quantifies preprocessing contributions.

\subsection{Deep Learning Approaches}

Deep neural networks~\cite{choetkiertikul2018deep,alqadi2021deep} have been explored for SEE, particularly with rich feature sets (telemetry, commits, process metrics).
\textbf{Strengths:} Representation learning; can model complex temporal/sequential patterns; promising for large-scale industrial data.
\textbf{Limitations:} SEE tabular datasets are typically small ($n{<}3000$); reproducibility challenges (architecture search, initialization sensitivity); require feature richness often unavailable in public corpora.
\textbf{How our work differs:} We focus on \emph{methodological fairness} (auditability, baselines, aggregation) for ensemble methods on public tabular data, not on claiming deep learning superiority.

\subsection{Transfer Learning and Cross-Project Generalization}

Recent work explores transfer learning~\cite{kocaguneli2013exploiting}, cross-company models, and domain adaptation to address distribution shift.
\textbf{Strengths:} Addresses cold-start problems; leverages external project data; relevant for organizations with limited historical data.
\textbf{Limitations:} Requires feature alignment across datasets; public data often lacks metadata for effective transfer; empirical gains mixed.
\textbf{How our work differs:} We employ leave-one-source-out (LOSO) validation for LOC schema as a first step toward cross-source generalization, explicitly reporting limitations rather than claiming full transfer learning.

\subsection{Emerging Approaches: Uncertainty, Fuzzy Logic, and Hybrid Methods}

Recent advances explore uncertainty quantification and hybrid architectures to improve estimation robustness~\cite{liu2024fuzzy,wang2025pattern,zhang2024uncertainty,chen2025hybrid}. Fuzzy logic methods~\cite{liu2024fuzzy} handle imprecise project parameters, while pattern recognition approaches~\cite{wang2025pattern} leverage deep feature extraction for complex effort relationships. Uncertainty-aware models~\cite{zhang2024uncertainty} provide confidence intervals alongside point estimates, addressing practitioner concerns about prediction reliability. Hybrid ensemble frameworks~\cite{chen2025hybrid} combine parametric and non-parametric methods to balance interpretability with accuracy. These methods show promise but often require substantial feature engineering and domain expertise. Our work complements this direction by establishing transparent baseline comparisons and reproducible protocols, enabling fair evaluation of such advanced techniques in future studies.
...
\midrule
Boehm (1981)~\cite{boehm1981software} & LOC & COCOMO calibration data (proprietary industrial projects) & Parametric (power-law + effort multipliers) & Hold-out test & No \\
Albrecht (1979) FP & FP & IBM internal projects (proprietary) & Linear regression & Retrospective validation & No \\
Karner (1993) UCP & UCP & Ericsson case studies (proprietary) & Weighted sum formula & Industrial case & No \\
\midrule
...
Wen et al. (2012)~\cite{wen2012systematic} & LOC & ISBSG, COCOMO81 (mixed public/restricted) & SVM, NN, GP & Hold-out test & Partial \\
...
\vspace{0.3em}
\raggedright\footnotesize
\textit{Note:} Public benchmarks (NASA93, COCOMO81, Desharnais) are documented in curated collections~\cite{jones2022estimation,rodriguez2023dase}. Original COCOMO (1981) calibration data was proprietary and distinct from the later-released NASA93 public benchmark. ISBSG repository imposes commercial licensing~\cite{isbsg2025overview}; we do not redistribute restricted data.
\end{table*}
...
three major paradigms: (i) traditional parametric models (COCOMO family), 
...
We have presented a \textbf{reproducible cross-schema benchmarking framework} for software effort estimation, establishing methodological infrastructure for fair comparison across LOC-, FP-, and UCP-based sizing paradigms.
Four concrete methodological contributions distinguish this work:
(1) \textbf{dataset manifest with provenance tracking} (Table~\ref{tab:dataset-summary}, Figure~\ref{fig:dataset-timeline}),
...
quantifying preprocessing component contributions.

\textit{Empirical demonstration:} Ensemble learners---most notably \textbf{Random Forest}---
achieved 42\% lower MMRE ($0.65 \pm 0.04$ vs. $1.12 \pm 0.08$) compared to
the calibrated parametric baseline, with low variance across multiple random train-test splits.
While these results corroborate findings in recent literature
on ensemble-based effort estimation~\cite{tanveer2023comprehensive,pandey2023comprehensive,alqadi2021deep}, the \emph{primary contribution is the benchmarking framework itself}, which future studies can adopt to evaluate new models or datasets under consistent, fair, and auditable conditions.
