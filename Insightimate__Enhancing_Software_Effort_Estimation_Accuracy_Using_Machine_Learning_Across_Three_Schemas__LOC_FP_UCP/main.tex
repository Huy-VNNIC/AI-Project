...
\begin{figure}[ht]
    ...
    \textbf{(1) Input:} Multi-source datasets (1993--2022) partitioned by schema (LOC/FP/UCP) with dataset manifest (Table~\ref{tab:dataset-manifest}). 
...
Despite rigorous deduplication rules (Table~\ref{tab:dataset-manifest}), residual near-duplicates may persist due to:
\begin{itemize}[leftmargin=1.5em]
...
\textbf{How our work differs:} We provide full dataset manifest (Table~\ref{tab:dataset-manifest}) with explicit deduplication and leakage controls; macro-averaging across schemas prevents LOC dominance; LOSO validation tests cross-source generalization; ablation quantifies preprocessing contributions.
