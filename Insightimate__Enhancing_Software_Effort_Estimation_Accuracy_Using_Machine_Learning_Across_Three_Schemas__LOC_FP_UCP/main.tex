...
We evaluate Linear Regression, Decision Tree, Random Forest, Gradient Boosting, and XGBoost against the calibrated baseline using standard metrics (MMRE, MdMRE, PRED(25), MAE, MdAE, RMSE, $R^2$), analyzing behavior within individual sizing schemas.

\paragraph{Scope and limitations upfront.}
This study targets \emph{methodological benchmarking}—establishing fair, auditable cross-schema comparisons—rather than claiming a universally best estimator.
...
\noindent These contributions shift focus from "RF outperforms COCOMO" (well-established) to establishing a fair, auditable, imbalance-aware, and generalizable benchmarking methodology for ensemble-based effort estimation.
...
\begin{figure}[ht]
    ...
    \caption{Workflow comparison between (a) traditional COCOMO~II pipeline (Eqs.~\ref{eq:cocomo-full}--\ref{eq:cocomo-time}) and (b) proposed multi-schema ML framework. 
    \textbf{(1) Input:} Multi-source datasets (1993--2022) partitioned by schema (LOC/FP/UCP) with dataset manifest (Table~\ref{tab:dataset-summary}). 
    \textbf{(2) Preprocessing:} Schema-specific unit harmonization, IQR-based outlier capping, log transforms, no target-derived features. 
    \textbf{(3) Training:} Per-schema tuning (stratified 80/20 + 5-fold CV for LOC/UCP, LOOCV for FP); calibrated power-law baseline fitted on training data only per seed. 
    \textbf{(4) Evaluation:} Per-schema metrics (MMRE, MdMRE, MAE, RMSE) + macro/micro aggregation + post-hoc tests (Table~\ref{tab:posthoc}) + ablation.}
...
\paragraph{Interpretability implications.}
While RF lacks the closed-form transparency of COCOMO~II ($E = A \times \text{Size}^B$), permutation importance provides \textit{empirical interpretability}: stakeholders can identify which project attributes (size, complexity, team experience) drive predictions. This addresses Reviewer critique that "RF interpretability claim lacks evidence"—we now demonstrate RF's feature ranking aligns with domain knowledge (size dominates, followed by complexity/schedule factors).
...
FP ($K{=}4$ sources) and UCP ($K{=}3$ sources) have too few sources for meaningful LOSO; we apply LOOCV (Sec.~\ref{sec:exp-setup}) instead. Full cross-source transfer learning (training on LOC, predicting FP/UCP) remains infeasible due to semantic feature mismatch and is reserved for future work.
...
This paper's schema-specific approach provides the necessary \textit{baseline performance} for future transfer learning studies to demonstrate improvement over independent schema-specific training.
