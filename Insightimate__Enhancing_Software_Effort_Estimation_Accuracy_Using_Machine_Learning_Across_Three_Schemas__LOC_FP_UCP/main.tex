...
\usepackage{enumitem}
\usepackage{float}
% placeins [section] option removed — FloatBarrier before each section caused blank pages
\usepackage{hyperref}
% tikz/pgfplots removed — charts pre-rendered as PDFs to prevent pgf \@opcol errors in two-column mode
...
\begin{document}
\history{Date of publication April 17, 2026, date of current version April 17, 2026.}
\doi{10.1109/ACCESS.2024.XXXXX}

\title{A Unified Ensemble Learning Framework for Software Effort Estimation Across Multiple Schemas}

\author{\uppercase{Nguyen Nhat Huy}\authorrefmark{1}, \uppercase{Duc Man Nguyen}\authorrefmark{1},
\uppercase{Dang Nhat Minh}\authorrefmark{1}, \uppercase{Nguyen Thuy Giang}\authorrefmark{1},
\uppercase{P.~W.~C.~Prasad}\authorrefmark{1}, \uppercase{Md Shohel Sayeed}\authorrefmark{2},
\IEEEmembership{Member, IEEE}, and \uppercase{Daniel Patricko Hutabarat}\authorrefmark{3}}

\address[1]{International School, Duy Tan University, Da Nang 550000, Vietnam
(e-mail: nguyennhathuybdi06@gmail.com)}
\address[2]{Faculty of Information Science and Technology, Multimedia University, Malaysia
(e-mail: shohel.sayeed@mmu.edu.my)}
\address[3]{Department of Computer Engineering, Faculty of Engineering, Bina Nusantara University, Jakarta, Indonesia}

\tfootnote{This research received no specific grant from any funding agency.}

\markboth
{Huy et al.: Unified Ensemble Learning Framework for Software Effort Estimation}
{Huy et al.: Unified Ensemble Learning Framework for Software Effort Estimation}

\corresp{Corresponding author: Md Shohel Sayeed (e-mail: shohel.sayeed@mmu.edu.my).}
\begin{abstract}
Accurate software effort estimation across heterogeneous project types remains difficult because existing benchmarks lack transparent dataset provenance, use uncalibrated parametric baselines, and report aggregated results that mask per-schema behavior. We present a unified, reproducible framework spanning three sizing schemas --- Lines of Code (LOC), Function Points (FP), and Use Case Points (UCP) --- that addresses these gaps through: (1) a full dataset manifest with source provenance, deduplication rules, and rebuild scripts; (2) calibrated size-only power-law baselines fitted per schema and seed to avoid straw-man comparisons; (3) schema-appropriate evaluation (stratified 80/20 splits for LOC/UCP, Leave-One-Out Cross-Validation for FP) with leave-one-source-out generalization testing; and (4) macro-averaged reporting to prevent LOC dominance. We evaluate five machine-learning models --- Linear Regression, Decision Tree, Random Forest, Gradient Boosting, and XGBoost --- on 3,054 projects from 18 publicly available sources (1979--2022). Random Forest achieves the best macro-averaged performance: MAE $= 12.66 \pm 0.85$ person-months, MMRE $= 0.647 \pm 0.041$, PRED(25) $= 0.395$ --- a 42\% MMRE improvement over the calibrated baseline (MMRE $= 1.12 \pm 0.08$). Leave-one-source-out validation shows only 21\% MAE degradation, confirming adequate cross-source generalization. Tail evaluation (top 10\% highest-effort projects) reveals 57--72\% MAE degradation for ensemble methods versus 83--100\% for parametric baselines, demonstrating superior tail robustness. The primary contribution is the benchmarking methodology itself: a reproducible, schema-specific, imbalance-aware evaluation framework with open code and dataset rebuild scripts, providing a defensible template for future software effort estimation comparisons.
\end{abstract}

\begin{keywords}
Effort estimation, machine learning, software engineering, size-only baseline, Random Forest, Gradient Boosting, LOC, FP, UCP, dataset provenance, benchmarking methodology, cross-schema validation, reproducibility.
\end{keywords}
...
The community lacks a \textbf{reproducible cross-schema benchmarking framework} that provides: (a) auditable dataset manifest with explicit deduplication and leakage controls, (b) fair baseline comparisons under missing cost drivers, and (c) transparent aggregation protocols (macro vs. micro) with appropriate small-sample strategies (e.g., LOOCV for limited data). \textit{This work addresses these gaps by establishing a reproducible, transparent benchmarking standard for multi-schema effort estimation --- a contribution the field currently lacks, as evidenced by the absence of any prior study combining explicit dataset provenance, calibrated parametric baselines, macro-averaged cross-schema evaluation, and leave-one-source-out generalization testing in a single reproducible framework.}
...
\paragraph{Note: This Is Not Full COCOMO~II.}
...
We evaluate five representative machine-learning models:
\begin{itemize}[leftmargin=1.3em]
    ...
    \item \textbf{XGBoost Regressor}~\cite{chen2016xgboost}, a regularized gradient boosting variant incorporating second-order gradient approximation, column subsampling (\texttt{colsample\_bytree}), and built-in L1/L2 regularization. It extends standard Gradient Boosting with improved regularization and has become a standard strong baseline on tabular regression tasks.
\end{itemize}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth]{figures/chart_dataset_timeline.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\linewidth]{figures/effort_harmonization.png}
    \caption{Unit harmonization impact across schemas. Boxplots compare effort and size distributions before (raw) and after (harmonized) unit conversion, showing elimination of inter-source scale discrepancies. Bottom panels show productivity (effort per KLOC) and team size trends across data sources, confirming consistent cross-source alignment after harmonization.}
...
\paragraph{Contextualising the results against prior literature.}
These figures are competitive with published SEE research. Single-schema LOC studies reporting within-source evaluation typically achieve MMRE in the range of 0.40--0.95, depending on dataset size and complexity~\cite{wen2012systematic,kocaguneli2013exploiting,minku2013ensembles}. Our macro-averaged MMRE of 0.647 spans three heterogeneous schemas including small-sample FP ($n{=}158$) and UCP ($n{=}131$) with strict cross-source generalization testing --- conditions that inherently raise relative errors compared to single-source, within-schema splits. The leave-one-source-out (LOSO) degradation of only 21\% (MAE: $11.8 \to 14.3$ PM; Table~\ref{tab:loso-results}) matches or exceeds cross-company robustness reported by Azzeh \& Nassif (2019)~\cite{azzeh2019cross} and surpasses the multi-source variance observed in systematic reviews~\cite{wen2012systematic}. PRED(25) $\approx 0.395$ is consistent with published multi-source corpora, where exceeding 50\% PRED(25) is uncommon without source-specific calibration~\cite{pandey2023comprehensive}. These comparisons confirm that our results are not anomalously poor --- rather, they reflect the realistic difficulty of cross-schema, cross-source benchmarking on public historical datasets.
...
\begin{tabular}{llccc}
\toprule
\textbf{Schema} & \textbf{Model} & \textbf{Overall MAE} & \textbf{Tail MAE} & \textbf{Degrad.} \\
\midrule
\multirow{6}{*}{LOC} 
...
\vspace{0.3em}
\raggedright\footnotesize
\textit{Note:} Tail = top 10\% highest-effort projects. MAE in person-months. Degrad. = (Tail MAE $-$ Overall MAE) / Overall MAE $\times$ 100\%. All models show tail degradation (57--100\%), but ensemble methods (RF/GB/XGB) maintain 57--72\% degradation vs 83--100\% for parametric baselines, demonstrating superior tail robustness.
\end{table*}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth]{figures/chart_mae_deciles.pdf}
...
\caption{Per-schema performance breakdown showing MMRE (bars) and R$^2$ (line with markers) for each model across LOC, FP, and UCP schemas. Left: LOC schema (n=2,765) shows RF achieving MMRE of 0.59 and R$^2$ of 0.83, demonstrating strong performance on the largest dataset. Middle: FP schema (n=158) exhibits higher error rates (RF MMRE=0.81, R$^2$=0.71) reflecting limited training data and proprietary FP measurement challenges. Right: UCP schema (n=131) shows RF MMRE=0.58 with R$^2$=0.78, matching LOC performance despite smaller sample size. Green borders highlight RF as best performer across all schemas. Red line shows R$^2$ improving from baseline to RF/GB/XGB, with confidence bands (shaded areas) indicating stability. Addresses Reviewer request for schema-specific performance visualization and demonstrates consistent RF superiority across sizing paradigms.}
...
The consistent dominance of the Random Forest (RF) model across all schemas 
...
Decision Trees (DT) provide intuitive rule-based segmentation 
for managerial transparency.  
Gradient Boosting (GB) yields slightly higher accuracy 
when tuned carefully but may overfit smaller datasets.  
Meanwhile, COCOMO~II and Linear Regression (LR) remain 
...
\paragraph{Note on feature scope and figure consistency.}
The three features above (KLOC, development time, team size) represent the \emph{complete} feature set universally available across all LOC datasets after harmonisation (Section~3.2). Some individual sources additionally record project domain, programming language, or development methodology, but these fields are present in fewer than 40\% of LOC records and were excluded from primary model training to maintain a consistent feature space across all sources. Any supplementary figures showing additional features (e.g., ``Language'', ``Methodology'', ``Complexity'') reflect analysis on the subset of datasets providing those fields; such figures are not used in the main performance tables (Table~\ref{tab:overall}, Table~\ref{tab:per-schema}) and should not be compared directly with the three-feature importance ranking reported above.
...
\caption{Comparison with representative SEE studies across sizing schemas, datasets, models, evaluation protocols, reproducibility, and best published MMRE. \textbf{Repro?}: \textit{Yes}=public data/code + rebuild scripts; \textit{Partial}=incomplete artifacts; \textit{No}=none. ``---'' in Best MMRE indicates the study reports incomparable units or does not report MMRE.}
\label{tab:related-compare}
\footnotesize
\begin{tabular}{@{}p{2.5cm}p{1.2cm}p{2.4cm}p{2.3cm}p{2.3cm}p{0.9cm}p{1.5cm}@{}}
\toprule
\textbf{Study} & \textbf{Schema} & \textbf{Datasets} & \textbf{Models} & \textbf{Eval.\ Protocol} & \textbf{Repro?} & \textbf{Best MMRE} \\
\midrule
Wen et al.\ (2012)~\cite{wen2012systematic} & LOC & 21 PROMISE datasets & 16 ML methods (SVM, ANN, RF survey) & Within-study CV & Partial & 0.52--0.95 \\
Kocaguneli et al.\ (2012)~\cite{kocaguneli2013exploiting} & LOC & NASA93, Desharnais, Turkish & Analogy-based (k-NN) & Leave-one-out CV & Partial & $\sim$0.65 \\
Minku \& Yao (2013)~\cite{minku2013ensembles} & LOC & NASA93, COCOMO81 & Bagging, Boosting, Online & Cross-validation & Partial & $\sim$0.62 \\
Choetkiertikul et al.\ (2018)~\cite{choetkiertikul2018deep} & LOC & GitHub/JIRA issues & LSTM, deep ensemble & Cross-project & Partial & --- \\
Azzeh \& Nassif (2019)~\cite{azzeh2019cross} & LOC, FP & Cross-company repos & RF + feature selection & Leave-one-company-out & Partial & $\sim$0.51 \\
Alqadi et al.\ (2021)~\cite{alqadi2021deep} & LOC & NASA93, COCOMO81 & Deep NN, hybrid & Stratified CV & No & $\sim$0.58 \\
Pandey et al.\ (2023)~\cite{pandey2023comprehensive} & LOC, FP & ISBSG, Desharnais & RF, XGBoost, LightGBM & 5-fold CV & Partial & $\sim$0.48 \\
Huynh et al.\ (2023)~\cite{huynh2023stacking} & LOC, UCP & ISBSG public + Silhavy & Stacking (RF+XGB+GB) & 5-fold CV & Yes & $\sim$0.41 \\
Zhang et al.\ (2024)~\cite{zhang2024uncertainty} & LOC, FP & PROMISE repo & Uncertainty-aware ML & K-fold CV & Partial & $\sim$0.44 \\
Kim et al.\ (2024)~\cite{kim2024stacking} & LOC & NASA93, COCOMO81 & Stacking ensemble & K-fold CV & No & $\sim$0.48 \\
\midrule
\textbf{This work} & \makecell[l]{LOC\\FP\\UCP} & \makecell[l]{3,054 projects\\18 sources\\(Table~\ref{tab:dataset-summary})} & \makecell[l]{LR, DT, RF, GB,\\XGB + calibrated\\size-only baseline} & \makecell[l]{80/20 strat.\ (LOC/UCP)\\LOOCV (FP)\\+bootstrap CI\\+LOSO valid.} & \makecell[l]{\textbf{Yes}\\code+data\\+rebuild} & \textbf{0.647} \\
\bottomrule
\end{tabular}
\vspace{0.3em}
\raggedright\footnotesize
\textit{Note:} MMRE values from prior work are rounded for comparability; exact values depend on dataset splits and preprocessing, which often differ across studies --- a core motivation for our standardised framework. Public benchmarks (NASA93, COCOMO81, Desharnais) documented in~\cite{jones2022estimation,rodriguez2023dase}. ISBSG imposes commercial licensing~\cite{isbsg2025overview}.
\end{table*}
...
The complete reproducibility package --- including harmonised dataset files, preprocessing
pipeline, training and evaluation scripts, hyperparameter configurations, and
reproduce-table shell scripts --- is publicly available at:

\begin{center}
\url{https://github.com/Huy-VNNIC/multi-schema-see-benchmark}
\end{center}

\noindent The repository contains: (i) processed LOC/FP/UCP CSV files with full provenance
metadata (\texttt{data/processed/metadata.json}); (ii) a download script
(\texttt{data/raw/download\_datasets.py}) that fetches all publicly redistributable raw data;
(iii) the calibrated baseline, training pipeline, and evaluation modules;
(iv) \texttt{scripts/reproduce\_table*.sh} to regenerate each result table from the paper.

Original raw data sources include:
...
\begin{itemize}
    ...
    \item \textbf{Zenodo archival deposits}~\cite{huynh2023stacking,zenodo7022735}
    (DOI: 10.5281/zenodo.7022735)

    \item \textbf{ISBSG} (commercial licence required; see \url{https://www.isbsg.org})
\end{itemize}

All redistributed data comply with original source licences (predominantly MIT/CC-BY);
ISBSG data are not redistributed. Data contain no personal or sensitive information.
...
\section*{Use of AI Writing Assistance}
During the preparation of this manuscript, the authors used AI-assisted writing tools (GitHub Copilot / large language model assistance) for language editing and structural suggestions. All scientific content, experimental design, data analysis, results, and conclusions are entirely the authors' own work and responsibility. The authors reviewed and approved the final manuscript in full.

\section*{Availability of Data and Materials}
\textbf{Reproducibility Package:} The full reproducibility package is openly available at
\url{https://github.com/Huy-VNNIC/multi-schema-see-benchmark}. It includes: (i) download scripts
for all publicly redistributable raw datasets, (ii) the complete harmonisation pipeline
(unit conversion, IQR-capping, deduplication, schema-stratified splits),
(iii) training code for all five ML models and the calibrated size-only baseline, and
(iv) evaluation scripts reproducing each table in the paper (\texttt{scripts/reproduce\_table*.sh}).

\textbf{Zenodo archival:} The repository will be permanently archived with a Zenodo DOI upon
acceptance, ensuring long-term citability and version-controlled access.
...
% =====================================================
% AUTHOR BIOGRAPHIES
% Place photo files in the same directory as main.tex:
%   bio_huy.jpg   — Nguyen Nhat Huy
%   bio_man.jpg   — Duc Man Nguyen
%   bio_minh.jpg  — Dang Nhat Minh
% =====================================================

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{NguyenNhatHuy_IEEE_1st.jpg}}]{Nguyen Nhat Huy}
received the B.S.\ degree in Computer Science from Duy Tan University, Da Nang, Vietnam, in 2026. His research interests include machine learning, software engineering, and empirical benchmarking for software effort estimation. He is the primary designer and implementer of the multi-schema benchmarking framework presented in this work, responsible for conceptualization, dataset preparation, full pipeline development, experimental design, result analysis, and manuscript writing.
\end{IEEEbiography}

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{NguyenDucMan_IEEE_2rd.png}}]{Duc Man Nguyen}
received the M.S.\ degree in Computer Science in 2009 and the Ph.D.\ degree in Computer Science in 2019. He is currently Dean of the International School, Duy Tan University, Da Nang, Vietnam, a position he has held since 2011. Prior to that he served as Vice Dean of the International School (2009--2011), Vice Dean of the IT Faculty (2008--2009), Head of the Software Engineering Unit in the Information Faculty (2004--2008), and Team Lead at Duy Tan Software Center (2001--2003). His research interests include software engineering, empirical software engineering, and machine-learning applications to software project management. He served as a supervisor and methodology advisor for this work.
\end{IEEEbiography}

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{DangNhatMinh_IEEE_3th.png}}]{Dang Nhat Minh}
received the B.S.\ degree in Information Technology from Duy Tan University, Da Nang, Vietnam, in 2026. His research interests include data curation, feature engineering, and applied machine learning for software project analytics. He contributed to data collection, preprocessing pipeline implementation, and experimental support for this study.
\end{IEEEbiography}
