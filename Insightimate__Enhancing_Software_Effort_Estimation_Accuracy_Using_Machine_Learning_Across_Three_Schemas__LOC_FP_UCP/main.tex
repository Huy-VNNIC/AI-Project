...
Models are trained \textbf{per schema} (LOC/FP/UCP independently) without cross-schema transfer learning. While this prevents information leakage and ensures fair schema-specific evaluation, it leaves unexplored whether LOC-trained models can transfer to FP/UCP contexts with appropriate feature mapping. Cross-schema transfer remains an open research question requiring careful alignment of size semantics (e.g., mapping FP complexity weights to LOC size brackets).
