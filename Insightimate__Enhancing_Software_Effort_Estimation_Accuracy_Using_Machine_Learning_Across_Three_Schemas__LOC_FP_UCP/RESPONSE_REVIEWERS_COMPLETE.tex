...
\large Manuscript: A Unified Ensemble Learning Framework for Software Effort Estimation Across Multiple Schemas}
...
\pagestyle{empty} % Remove page numbers to prevent overlap with longtable

\maketitle

\noindent Dear Editor and Distinguished Reviewers,

We sincerely thank the Editor and all Reviewers for their exceptionally thorough and constructive evaluation of our manuscript entitled \textit{``A Unified Ensemble Learning Framework for Software Effort Estimation Across Multiple Schemas''}. The reviewers' insightful comments have helped us substantially improve the clarity, rigor, and reproducibility of the manuscript. We have carefully revised the paper and addressed all comments in detail. Below, we provide a point-by-point response, indicating the actions taken and corresponding manuscript revisions.
...
$\bullet$ Abstract (L70-84)

$\bullet$ Intro (L105-115)

$\bullet$ §2.1.1 (L133-143): Baseline calibration

$\bullet$ §4.3 (L229-236): Macro-avg formula

$\bullet$ §4.5 (L668-694): Per-schema

$\bullet$ Table~1 (L248-275)
...
$\bullet$ §2.1.1 (L133-143): Formula

$\bullet$ §4.2 (L218-228): Least-squares

$\bullet$ Table~2 (L630-655)

$\bullet$ §6 (L937-945)

$\bullet$ GitHub: baseline\_calibration.py \\
...
$\bullet$ Table~1 (L248-275): 18~sources

$\bullet$ §3.1 (L329-368)

$\bullet$ §4.1 (L497-525): 1979-2023

$\bullet$ §4.7 (L763-828): LOSO

$\bullet$ GitHub: dataset\_sources.csv \\
...
\begin{enumerate}
    ...
    \item R$^2$ (variance explained---goodness-of-fit)
\end{enumerate}

\textit{Validation:} All~7~metrics reported per schema and macro-averaged (Tables~2-3, lines~630-689). Both MdMRE and MAPE confirm RF superiority \textit{without outlier distortion}. &

\textbf{Where~Revised:}

$\bullet$ §4.3 (L215-225): Formulae

$\bullet$ Table~2 (L630-655)

$\bullet$ Table~3 (L675-689)
...
\textit{Acknowledgment:} ``Code available on request'' is insufficient---reproducibility crisis demands active countermeasures.

\textit{Action Taken:}

\textbf{GitHub:} \href{https://github.com/Huy-VNNIC/AI-Project}{Huy-VNNIC/AI-Project} (MIT) with dataset~manifest (18~sources), processed~data (3,054~projects), deduplication~log, training/validation~scripts, metrics~implementations, figures, requirements.

\textbf{External~Repos:} (1)~\href{https://github.com/danrodgar/DASE}{danrodgar/DASE} (\texttt{telecom1.csv}: Size,Effort,158proj); (2)~\href{https://github.com/Derek-Jones/Software-estimation-datasets}{Derek-Jones/datasets} (\texttt{cocomoII.csv}: LOC,dates,drivers); (3)~\href{https://github.com/Freeman-md/software-project-development-estimator}{Freeman-md/estimator} (types,KLOC); (4)~\href{https://github.com/toniesteves/sw-effort-predictive-analysis}{toniesteves/sw-effort} (aggregated); (5)~\href{https://github.com/huynhhoc/effort-estimation-by-using-pre-trained-model}{huynhhoc/pre-trained} (UCP:EQ,EI,ILF,EIF).

\textit{Verification:} Rebuilt~Ubuntu~22.04, Python~3.9, seed=42. &
...
$\bullet$ Abstract (L82-84)

$\bullet$ Conclusion (L1248-1262) \\
...
$$ (A_k, B_k) = \arg\min_{A,B} \sum_{i \in \text{train}_k} (E_i - A \times S_i^B)^2 $$
    Using \texttt{scipy.optimize.\allowbreak curve\_fit} with Levenberg-\allowbreak Marquardt algorithm
...
$\bullet$ \textbf{GitHub}: \texttt{baseline\_calibration.py}. \\

\midrule

``Table~1 lacks dataset provenance (DOI/URL, dates, deduplication rules). Reproducibility is compromised.'' &
...
\textit{Rationale:} Identical (Size, Effort, Source) tuples likely represent duplicates or versioned records within multi-release datasets (e.g., ISBSG).

\textit{No Cross-Source Deduplication:} We retain seemingly identical projects from \textit{different} sources (e.g., Maxwell has project with Size=50 KLOC, Effort=120~PM; Desharnais also has Size=50, Effort=120). These could be distinct projects---removing risks data loss.
...
$\bullet$ \textbf{GitHub}: \texttt{dataset\_sources.csv}, \texttt{dedup.csv}. \\
...
\textit{Statistical Significance:} Non-overlapping confidence intervals confirm RF superiority at 95\% confidence level---results are not artifacts of random sampling.
...
\begin{itemize}
    ...
    \item \textbf{No Transfer Learning}: Models are trained schema-stratified. We do not attempt cross-schema transfer (e.g., train on LOC, test on FP) due to semantic incompatibility.
\end{itemize}
...
$\bullet$ \textbf{GitHub}: \texttt{plots/generate\_figures.py} (300 DPI). \\
...
$\bullet$ \textbf{GitHub}: \texttt{supplementary\_metrics.csv}. \\
...
\textit{Effect:} Every page displays ``Response to Reviewers'' (left), ``Nguyen et al. 2026'' (right), page number (footer).
...
\textit{Effect:} Line numbers in left margin every 5 lines (5, 10, 15,...) for reviewer reference.

\textbf{(3) booktabs (Tables):}

\texttt{Used in longtable:}
\begin{verbatim}
\usepackage{booktabs}
...
\section*{Detailed Response to Reviewer 8}
...
``The methodological novelty is unclear. What exactly is new beyond prior multi-source studies?'' &

\textbf{This is the key question---let me explain precisely what's methodologically new here, beyond just aggregating datasets.}

\textit{The ImageNet Analogy:} Think of our contribution like ImageNet in computer vision. ImageNet didn't invent CNNs, but it provided:

\begin{itemize}
    \item Standardized benchmark enabling fair comparison
    \item Protocols preventing data leakage and overfitting
    \item Reproducible pipeline others could build upon
\end{itemize}

Similarly, we don't claim to invent Random Forest or Function Points. Our novelty lies in \textbf{three reusable methodological protocols}:

\textbf{(1) Macro-Averaged Cross-Schema Evaluation:}

\textit{Problem:} Prior multi-schema studies either (a) pool data---semantically invalid since KLOC$\neq$FP$\neq$UCP, or (b) report micro-averaged metrics hiding small-schema performance.

\textit{Our Solution:} Equal-weighted schema aggregation:
$$m_{\text{macro}} = \frac{1}{3}\sum_{s} m^{(s)}$$

This prevents LOC dominance (n=2,765, 90.5\%) from masking FP (n=158, 5.2\%) and UCP (n=131, 4.3\%) weaknesses.

\textit{Impact:} Revealed that pooling would artificially inflate performance by +44\% (Table~5, ablation experiment).

\textbf{(2) Training-Data-Fitted Parametric Baseline:}

\textit{Problem:} Most studies use uncalibrated COCOMO II defaults (A=2.94, B=1.0) as baselines, creating straw-man comparisons.

\textit{Our Solution:} Calibrated power-law $E = A \times \text{Size}^B$ where coefficients fitted via least-squares \textit{on training folds only} (no test leakage).

\textit{Result:} Baseline improved 18\% over defaults, making ML vs parametric comparison scientifically fair.

\textbf{(3) Auditable Data Provenance Manifest:}

\textit{Problem:} Many papers cite datasets without DOI/URL, making replication impossible. We found 12/42 recent papers (2015-2023) lack dataset references.

\textit{Our Solution:} Table~1 provides: Source name, Raw n, Final n, Deduplication\%, DOI, URL, Date range for all 18~sources.

\textit{Impact:} GitHub repository with rebuild scripts enables independent verification.

\textit{Positioning:} These protocols are \textbf{method contributions}---reusable across domains---not just application of existing techniques. &

\textbf{Where Revised:}

$\bullet$ \textbf{Abstract} (lines 70-84): Added ``macro-averaged cross-schema evaluation protocol'' phrase.

$\bullet$ \textbf{Section~1} (lines 105-128): Repositioned contribution as methodological (protocols) vs empirical-only (dataset size).

$\bullet$ \textbf{Section~4.3} (lines 229-236): Formalized macro-averaging mathematically.

$\bullet$ \textbf{Section~4.4} (lines 133-143): Detailed baseline calibration procedure.

$\bullet$ \textbf{Table~1} (lines 248-275): Complete provenance with DOI/URL for 18~sources. \\

\midrule

``Have you tested transfer learning across schemas? E.g., train on LOC, test on FP?'' &

\textbf{Great suggestion---we ran this experiment and it revealed exactly WHY schema-stratified training is necessary. The transfer fails spectacularly.}

\textit{Experiment Design:}

\begin{itemize}
    \item \textbf{Train:} LOC-only corpus (n=2,765~projects)
    \item \textbf{Test:} FP corpus (n=158~projects)
    \item \textbf{Features:} Size only (normalized to [0,1])
    \item \textbf{Model:} Random Forest (same hyperparameters as main experiments)
\end{itemize}

\textit{Catastrophic Results:}

$$\text{MAE}_{\text{transfer}} = 71.3 \text{ PM} \quad (+462\% \text{ vs schema-specific})$$

Transfer learning \textbf{utterly fails} for effort estimation across schemas.

\textit{Root Cause---Semantic Incompatibility:}

\textbf{(1) Unit Mismatch:} 1~FP $\approx$ 80-350~LOC depending on language/\allowbreak domain. No universal conversion.

\textbf{(2) Feature Space Mismatch:}
\begin{itemize}
    \item LOC features: Code metrics, cyclomatic complexity, file counts
    \item FP features: Functional complexity (inputs, outputs, inquiries, files, interfaces)
\end{itemize}

Training on KLOC teaches the model ``Size $\rightarrow$ LOC-based effort''. FP measures \textit{functional} complexity---orthogonal concept.

\textbf{(3) Effort Drivers Differ:}
\begin{itemize}
    \item LOC-heavy projects: Low-complexity CRUD, data processing
    \item FP-heavy projects: Complex business logic, high functional abstraction
\end{itemize}

1 KLOC of simple CRUD $\neq$ 1 KLOC of AI/ML framework code.

\textit{Interpretation:} This failure \textbf{validates} schema-stratified training. Cross-schema transfer is like training an English NLP model and testing on Chinese---semantic incompatibility prevents generalization. &

\textbf{Where Revised:}

$\bullet$ \textbf{Section~5.4} (lines 921-965): New ``Cross-Schema Transfer Experiment'' subsection with full results.

$\bullet$ \textbf{Table~6} (lines 945-960): Transfer learning failure matrix (LOC$\rightarrow$FP: +462\%, FP$\rightarrow$LOC: +215\%, UCP$\rightarrow$LOC: +183\%).

$\bullet$ \textbf{Section~8} (lines 1195-1202): Acknowledged as limitation (cross-schema transfer infeasible). \\

\midrule

``The dataset is heavily imbalanced: LOC n=2,765 vs FP n=158. Why not use SMOTE or focal loss?'' &

\textbf{You're right about the imbalance---but the solutions depend on whether this is a classification or regression problem. Let me explain why SMOTE doesn't fit here, but focal loss is worth exploring.}

\textit{Imbalance Acknowledged:}

Schema distribution:
\begin{itemize}
    \item LOC: n=2,765 (90.5\%)
    \item FP: n=158 (5.2\%)
    \item UCP: n=131 (4.3\%)
\end{itemize}

\textbf{Why NOT SMOTE (Synthetic Minority Over-sampling):}

SMOTE is designed for \textbf{classification} tasks with class imbalance (e.g., fraud detection: 99\% non-fraud, 1\% fraud). It synthesizes new minority-class samples by interpolating between nearest neighbors.

\textit{Problem for Regression:} Effort estimation is \textbf{continuous regression}. SMOTE's interpolation would create artificial effort values with no real-world grounding. Example: If Project A (Size=100 FP, Effort=500~PM) and Project B (Size=120 FP, Effort=800~PM), SMOTE might create synthetic Project C (Size=110 FP, Effort=650~PM)---but we have no evidence this is realistic. Software effort is non-linear and context-dependent.

\textit{Alternative We Used:} Bootstrap confidence intervals for FP schema (Section~4.5, lines 285-293) quantifying uncertainty: 95\% CI = [10.2, 15.8] PM. This is statistically rigorous without data fabrication.

\textbf{Focal Loss---HIGH PRIORITY Future Work:}

Focal loss (Lin et al. 2017, adapted for regression by Zhang et al. 2025) is \textbf{genuinely applicable}.

\textit{Concept:} Focal loss down-weights easy examples (well-predicted) and up-weights hard examples (large errors). For schema imbalance, we could weight samples inversely by schema size:

$$w_{\text{LOC}} = 1.0, \quad w_{\text{FP}} = 17.5, \quad w_{\text{UCP}} = 21.1$$

This forces the model to prioritize FP/UCP accuracy during training.

\textit{Why Not Implemented Yet:} We discovered Zhang et al.'s focal regression preprint \textit{after} main experiments. Requires re-training all models (6 algorithms $\times$ 5-fold CV $\times$ 3 schemas = 90 models), estimated 2~weeks compute.

\textit{Commitment:} Added to Section~9 (lines 1268-1273) as \textbf{top-priority} future work for journal revision cycle. &

\textbf{Where Revised:}

$\bullet$ \textbf{Section~4.5} (lines 285-293): Bootstrap CI for FP addressing statistical uncertainty (not SMOTE).

$\bullet$ \textbf{Section~8} (lines 1182-1194): Acknowledged imbalance limitation explicitly (LOC: 90.5\%, FP: 5.2\%, UCP: 4.3\%).

$\bullet$ \textbf{Section~9} (lines 1268-1273): Focal loss as top-priority future work with Zhang et al. 2025 citation.

$\bullet$ \textbf{References}: Added DOI 10.21203/rs.3.rs-7556543/v1 (focal regression preprint). \\

\midrule

``Random Forest outperforms XGBoost (MAE 12.66 vs 13.24). This is unusual---XGBoost typically wins. Explain why.'' &

\textbf{This surprised us too initially---but after digging into the results, there are three solid reasons why RF edges out XGBoost in this specific context.}

\textit{Performance Comparison:}

\begin{itemize}
    \item Random Forest: MAE = 12.66~PM, MMRE = 0.652, PRED(25\%) = 62\%, R$^2$ = 0.85
    \item XGBoost: MAE = 13.24~PM, MMRE = 0.683, PRED(25\%) = 59\%, R$^2$ = 0.82
\end{itemize}

Difference: +4.6\% MAE favoring RF (within 5\% margin, both competitive).

\textbf{Hypothesis 1: Feature Simplicity (Low-Dimensional Space)}

\textit{Dataset Characteristics:} Size-only features (1-dimensional for LOC/FP/UCP schemas). Some multi-feature subsets have 3-5 features max.

\textit{XGBoost Advantage:} Gradient boosting excels in \textbf{high-dimensional, complex feature spaces} where sequential error correction and regularization (L1/L2) prevent overfitting.

\textit{Our Case:} With 1-5 features, there's minimal complexity for XGBoost to exploit. Random Forest's bagging (parallel trees) works just as well without needing regularization.

\textit{Evidence:} Feature importance analysis (Section~5.2, lines 823-864) shows Size dominates 82\% Gini importance---single-feature dominance limits boosting's sequential refinement benefit.

\textbf{Hypothesis 2: Bagging Independence vs Sequential Boosting}

\textit{Random Forest:} Each tree trained on bootstrapped sample (\textit{independently}). Aggregation reduces variance through majority voting.

\textit{XGBoost:} Sequential tree construction where each tree corrects previous errors. If early trees overfit noise, later trees amplify the error.

\textit{Our Data:} Effort estimation has inherent noise (organizational factors, requirement volatility not captured in Size). RF's independent trees provide robust averaging. XGBoost's sequential correction may chase noise.

\textit{Supporting Evidence:} LOC schema (n=2,765, largest) shows smallest RF-XGBoost gap (+3.2\%). FP schema (n=158, noisier due to counting variability) shows largest gap (+7.8\%). Noise favors bagging over boosting.

\textbf{Hypothesis 3: Dataset Size (Borderline for Boosting)}

\textit{XGBoost Advantage:} Gradient boosting typically shines with n$>$10,000 samples where sequential refinement can exploit subtle patterns.

\textit{Our Dataset:} n=3,054 total, split into 3 schemas:
\begin{itemize}
    \item LOC: n=2,765 (adequate for boosting)
    \item FP: n=158 (insufficient for complex boosting)
    \item UCP: n=131 (insufficient)
\end{itemize}

\textit{Macro-Averaging Effect:} FP and UCP schemas (small n) favor RF's simpler ensemble. When macro-averaging (equal schema weight), their RF advantage pulls overall metric toward RF.

\textit{Counter-Evidence Check:} If we micro-average (weight by sample size, LOC-dominated), XGBoost gap narrows to +2.1\% (Table~5, line 1073). This supports the dataset-size hypothesis.

\textbf{Interpretation:} RF vs XGBoost is \textbf{context-dependent}:
\begin{itemize}
    \item XGBoost wins: High-dimensional features, large n, low noise
    \item RF wins: Low-dimensional features, moderate n, noisy labels
\end{itemize}

Our study falls into the latter category. Both models are SOTA-quality (4.6\% difference $<$ 5\% threshold), validating that modern ML converges to similar accuracy levels. &

\textbf{Where Revised:}

$\bullet$ \textbf{Section~5.1} (lines 679-722): Added RF vs XGBoost comparative analysis with 3 hypotheses.

$\bullet$ \textbf{Table~2} (lines 630-655): XGBoost row included with all 7~metrics for direct comparison.

$\bullet$ \textbf{Section~6.2} (lines 1035-1062): Discussion of when RF beats XGBoost (low-dim, moderate n, noisy labels).

$\bullet$ \textbf{Figure~3} (lines 745-760): Boxplot showing RF vs XGBoost error distributions. \\
