...
$$\text{MAE}_{\text{transfer}} = 71.3 \text{ PM} \quad (+462\% \text{ vs schema-specific } 12.7 \text{ PM})$$

Transfer learning \textbf{utterly fails} for effort estimation across schemas.

\textit{Root Cause---Semantic Incompatibility:}

\textbf{(1) Unit Mismatch:} 1 Function Point $\approx$ 80-350 LOC depending on language/domain. No universal conversion exists.
...
Focal loss (Lin et al. 2017, adapted for regression by Zhang et al. 2025\footnote{Zhang, Q. et al. (2025). Focal Regression Loss for Imbalanced Datasets. \textit{Preprint}. DOI: 10.21203/rs.3.rs-7556543/v1}) is \textbf{genuinely applicable}.
...
$$w_{\text{LOC}} = 1.0, \quad w_{\text{FP}} = \frac{2765}{158} = 17.5, \quad w_{\text{UCP}} = 21.1$$

This forces the model to prioritize FP/UCP accuracy during training.

\textit{Why Not Implemented Yet:} We discovered Zhang et al.'s focal regression preprint \textit{after} main experiments concluded. Requires re-training all models (6 algorithms $\times$ 5-fold CV $\times$ 3 schemas = 90 models), estimated 2 weeks compute time.
